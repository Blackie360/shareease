...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
    ...
    try {
      ...
      if (response.ok) {
        setResult({
          success: true,
          mode: data.mode,
          message: data.mode === "simulation" ? "Email simulation successful!" : "Email sent successfully!",
          details: data,
        })
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Comprehensive Email Testing</CardTitle>
      ...
      <CardContent>
                  ...
                  {result.details && (
                    <div className="mt-2 space-y-1 text-sm">
                      <p>
                        ...
                        {result.mode === "simulation" ? "Simulation" : "Real Email"}
                      </p>
                      <p>
        ...
        <div className="space-y-4 mb-6">
          <div className="space-y-2">
            ...
            <RadioGroup value={testMode} onValueChange={setTestMode} className="flex space-x-4" id="testMode">
            ...
            <p className="text-xs text-muted-foreground">
              {testMode === "simulation"
                ? "Simulates sending an email without actually sending it (works in all environments)"
                : "Attempts to send a real email (only works in production environments)"}
            </p>
          </div>
        ...
        <Button onClick={handleTestEmail} disabled={isLoading} className="w-full">
          {isLoading ? (
            <>
              ...
              {testMode === "simulation" ? "Simulating..." : "Sending..."}
            </>
          ) : (
            <>
              <Mail className="mr-2 h-4 w-4" />
              {testMode === "simulation" ? "Simulate Email" : "Send Real Email"}
            </>
          )}
        </Button>
      </CardFooter>
